...
for set in sets:  # Proposed
    cmd = f'python3 main.py --dataset cifar10 --noise_rate 0.3 --lam 0.0001 --reg_type min --loss_func sl --alpha {set[0]} --beta {set[1]} --percent_instance_noise {p} --sess 0'
    subprocess.run(cmd.split())


# for p in percent_instance:
#     for i in range(5):  # Proposed
#         cmd = f'python3 main.py --dataset cifar10 --noise_rate 0.3 --lam 0.0001 --reg_type min --loss_func sl --alpha 0.9 --beta 0.4 --percent_instance_noise {p} --sess {i}'
#         subprocess.run(cmd.split())
